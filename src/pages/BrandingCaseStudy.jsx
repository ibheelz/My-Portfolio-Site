  ...
  return (
    <div className="min-h-screen bg-[#06080a] p-[clamp(12px,3vw,24px)] lg:p-[clamp(6px,1.5vw,12px)] animate-fadeIn relative flex flex-col branding-page-container branding-mobile-fixed" style={{ ['--nav-h']: 'clamp(72px, 12vh, 120px)' }}>
      ...
      <style jsx>{`
        ...
        /* Lightbox styles */
        .lightbox-overlay { background-image: linear-gradient(rgba(0,0,0,0.62), rgba(0,0,0,0.62)), url(${csBG}); background-size: cover; background-position: center; backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px); display: flex; flex-direction: column; opacity: 0; transition: opacity 120ms ease; overflow: hidden; overscroll-behavior: contain; }
        .lightbox-fade-in { opacity: 1; }
        .lightbox-fade-out { opacity: 0; }
        .lightbox-modal { position: relative; width: 100%; height: 100%; display: flex; flex-direction: column; background: transparent; overflow: hidden; color: #e7f2f8; transform: scale(0.98); transform-origin: center center; transition: transform 120ms ease; opacity: 0; }
        ...
        .lightbox-image-wrap { display: flex; align-items: center; justify-content: center; flex: 1; padding: 0; touch-action: none; }
        .lightbox-image { max-width: 100%; max-height: 100%; object-fit: contain; object-position: center; border-radius: 0; box-shadow: none; will-change: transform, opacity, filter; }
        ...
        @media (max-width: 768px) { .lightbox-modal { width: 100%; height: 100%; } .lightbox-chevron { display: none; } }
      `}</style>
    </div>
  )
}
