  ...
  return (
    <div className="min-h-screen bg-[#06080a] p-[clamp(12px,3vw,24px)] lg:p-[clamp(6px,1.5vw,12px)] animate-fadeIn relative flex flex-col branding-page-container branding-mobile-fixed" style={{ ['--nav-h']: 'clamp(72px, 12vh, 120px)' }}>
      ...
      <style jsx>{`
        ...
        .lightbox-modal { position: relative; width: min(90vw, 1400px); max-height: 100vh; background: rgba(20,20,22,0.2); border-radius: 16px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.35); color: #e7f2f8; transform: scale(0.98); transform-origin: center center; transition: transform 120ms ease; opacity: 0; }
        ...
        .lightbox-image { max-width: 100%; max-height: calc(100vh - 130px); object-fit: cover; object-position: top center; border-radius: 12px; box-shadow: 0 6px 18px rgba(0,0,0,0.35); will-change: transform, opacity, filter; }
        ...
        @media (max-width: 768px) { .lightbox-modal { width: min(96vw, 1000px); max-height: 100vh; } .lightbox-image-wrap { padding: 12px 12px 90px; } .lightbox-chevron { display: none; } }
      `}</style>
    </div>
  )
}
